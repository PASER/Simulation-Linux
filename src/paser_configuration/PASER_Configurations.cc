...
//    }
    ...
    if (netDeviceNumber > 0) {
        ...
        netDevice[0].ipaddr.S_addr =
                pModul->PUBLIC_getInterfaceEntry(1)->ipv4Data()->getIPAddress().getInt();
    ...
    for (int i = 0; i < pModul->MYgetNumInterfaces(); i++) {
        ...
        if (memcmp(tempIf->getName(), "eth0", 4) == 0) {
            ...
            netEthDevice[0].ipaddr.S_addr =
                    pModul->PUBLIC_getInterfaceEntry(i)->ipv4Data()->getIPAddress().getInt();
...
//    strcpy(netDevice[1].ifname, pModul->PUBLIC_getInterfaceEntry(3)->getName());
    ...
    addressOfKDC.S_addr = ((in_addr_t) 0x0A0A0001);
...
//        netAddDevice[i].bcast.S_addr = (in_addr_t) 0xFFFFFF00;
        netAddDevice[i].mask.S_addr = paser_modul->PUBLIC_getInterfaceEntry(
                i + netDeviceNumber + 1)->ipv4Data()->getNetmask().getInt();
        netAddDevice[i].ipaddr.S_addr =
                paser_modul->PUBLIC_getInterfaceEntry(i + netDeviceNumber + 1)->ipv4Data()->getIPAddress().getInt()
                        & netAddDevice[i].mask.S_addr.getIPAddress().getInt()/*(in_addr_t) 0xFFFFFF00*/;
        ev << "net Mask: "
...
bool PASER_Configurations::isAddInMySubnetz(struct in_addr Addr) {
    for (u_int32_t i = 0; i < netAddDeviceNumber; i++) {
        ev << "(netAddDevice[i].mask.S_addr & Addr.S_addr) = "
                << (netAddDevice[i].mask.S_addr & Addr.S_addr) << "\n";
        if ((netAddDevice[i].mask.S_addr & Addr.S_addr)
                == (netAddDevice[i].mask.S_addr & netAddDevice[i].ipaddr.S_addr)) {
            return true;
        }
    }
    return false;
