...
//    }
    ...
    for (u_int32_t i = 0; i < paser_configuration->getNetAddDeviceNumber();
        ...
        ev << "addr: " << tempRange.ipaddr.S_addr.getIPAddress().str()
                << " mask: " << tempRange.mask.S_addr.getIPAddress().str()
                << "\n";
...
void PASER_Global::generateGwSearchNonce() {
    ...
    lastGwSearchNonce |= (u_int32_t) buf & 0x000000FF;
    while (!RAND_bytes(&buf, 4)) {
    };
    lastGwSearchNonce |= ((u_int32_t) buf << 8) & 0x0000FF00;
    while (!RAND_bytes(&buf, 4)) {
    };
    lastGwSearchNonce |= ((u_int32_t) buf << 16) & 0x00FF0000;
    while (!RAND_bytes(&buf, 4)) {
    };
    lastGwSearchNonce |= ((u_int32_t) buf << 24) & 0xFF000000;
}
...
void PASER_Global::resetPASER() {
    //Reset rreq_list
    for (std::map<Uint128, message_rreq_entry *>::iterator it =
