...
//    return 0;
    ...
    std::map<Uint128, message_rreq_entry *>::iterator it = rreq_list.find(
...
message_rreq_entry* PASER_RREQ_List::pending_find_addr_with_mask(
    ...
    for (std::map<Uint128, message_rreq_entry *>::iterator it =
        ...
        message_rreq_entry *entry = it->second;
        if ((tempAddr & dest_mask.S_addr)
                == (dest_addr.S_addr & dest_mask.S_addr)) {
            return entry;
        }
    }
    return NULL;
//    std::map<Uint128, message_rreq_entry * >::iterator it = rreq_list.find(dest_addr.s_addr);
//    if (it != rreq_list.end())
//    {
//        message_rreq_entry *entry = it->second;
//        if (entry->dest_addr.s_addr == dest_addr.s_addr)
