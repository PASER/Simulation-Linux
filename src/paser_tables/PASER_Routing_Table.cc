...
PASER_Routing_Table::~PASER_Routing_Table() {
    for (std::map<Uint128, PASER_Routing_Entry*>::iterator it =
...
PASER_Routing_Entry *PASER_Routing_Table::findAdd(struct in_addr addr) {
    for (std::map<Uint128, PASER_Routing_Entry*>::iterator it =
        ...
        for (std::list<address_range>::iterator it2 = tempEntry->AddL.begin();
            ...
            if ((tempRange.ipaddr.S_addr & tempRange.mask.S_addr)
                    == (addr.S_addr & tempRange.mask.S_addr)) {
                return tempEntry;
            }
        }
    }
    return NULL;
//    std::map<Uint128, PASER_Routing_Entry*>::iterator it = route_table.find(addr.s_addr);
...
//        return route_to_gw;
    ...
    std::map<Uint128, PASER_Routing_Entry*>::iterator it = route_table.find(
            dest_addr.s_addr);
    if (it != route_table.end()) {
        if (it->second)
            return it->second;
...
//    ev << "try to find best route to GW, routingTable.size = " << route_table.size() << "\n" ;
    for (std::map<Uint128, PASER_Routing_Entry*>::iterator it =
            route_table.begin(); it != route_table.end(); it++) {
//        ev << "IP: " << (*it).second->dest_addr.S_addr.getIPAddress().str() << " isGW: " << (int)((*it).second->is_gw) << " isValid: " << (int)((*it).second->isValid) << " metric: " << (int)(*it).second->hopcnt << "\n";
...
void PASER_Routing_Table::updateKernelRoutingTable(struct in_addr dest_addr,
    ...
    if (!del_entry) {
        ...
        if (tempRout && tempRout->dest_addr.S_addr == dest_addr.S_addr) {
            ...
            if (tempNeigh
                ...
                destAdd.S_addr = (in_addr_t) 0x00000000;
                struct in_addr destAddMask;
                destAddMask.S_addr = (in_addr_t) 0x00000000;
                ev << "update KernelTable: destAddr: "
                        << destAdd.S_addr.getIPAddress().str() << ", destMask: "
                        << destAddMask.S_addr.getIPAddress().str() << "\n";
                paser_modul->MY_omnet_chg_rte(destAdd.S_addr, forw_addr.S_addr,
                        destAddMask.S_addr, metric + 1, true, ifIndex);
                paser_modul->MY_omnet_chg_rte(destAdd.S_addr, forw_addr.S_addr,
    ...
    if (del_entry) {
        ev << "Loesche Entry: " << dest_addr.S_addr.getIPAddress().str()
                << ", gw: " << forw_addr.S_addr.getIPAddress().str() << "\n";
    }
...
void PASER_Routing_Table::updateRoutingTableAndSetTableTimeout(
        ...
        X509 *cert, struct in_addr nextHop, u_int8_t metric, int ifIndex,
    ...
    if (entry != NULL) {
        ...
        ev << "update Route in Routing Table for src: "
                << src_addr.S_addr.getIPAddress().str() << " over: "
                << nextHop.S_addr.getIPAddress().str() << "\n";
...
void PASER_Routing_Table::updateRoutingTable(struct timeval now,
    ...
    for (std::list<address_list>::iterator it = addList.begin();
        ...
        ev << "updating Kernel Routing Table from AddL, dest: "
                << tempList.ipaddr.S_addr.getIPAddress().str() << ", metric: "
                << hopCount << "\n";
...
void PASER_Routing_Table::deleteFromKernelRoutingTableNodesWithNextHopAddr(
    ...
    ev << "Loesche alle Knoten, die ueber "
            << nextHop.S_addr.getIPAddress().str() << " erreichbar sind\n";
    ...
    PASER_Neighbor_Entry *nEntry = paser_global->getNeighbor_table()->findNeigh(
            nextHop);
    if (nEntry) {
        ev << "Loesche den Nachbar " << nextHop.S_addr.getIPAddress().str()
                << " aus neighborTable\n";
        nEntry->isValid = 0;
    ...
    ev << "Loesche den Knoten " << nextHop.S_addr.getIPAddress().str()
            << " selbst\n";
    ev << "delete addr: " << rEntry->dest_addr.S_addr.getIPAddress().str()
            << "\n";
...
void PASER_Routing_Table::updateRouteLifetimes(struct in_addr dest_addr) {
    ...
    ev << "ip Addr: " << validPack->destAddr.S_addr.getIPAddress().str()
            << "\n";
    ev << "now: " << now.tv_sec << "\nRoute delete timeout: "
            << deletePack->timeout.tv_sec << "\n";
    ev << "Route valid timeout: " << validPack->timeout.tv_sec << "\n";
    ...
    ev << "ip Addr: " << NvalidPack->destAddr.S_addr.getIPAddress().str()
            << "\n";
    ...
    ev << "ip Addr: " << validRoutingPack->destAddr.S_addr.getIPAddress().str()
            << "\n";
    ev << "now: " << now.tv_sec << "\nRoute delete timeout: "
...
std::list<address_list> PASER_Routing_Table::getNeighborAddressList(int ifNr) {
    ...
    for (std::map<Uint128, PASER_Routing_Entry*>::iterator it =
        ...
        if (rEntry->hopcnt == 1 && nEntry != NULL && nEntry->neighFlag
                && nEntry->isValid) {
            address_list temp;
//            ev << "add IP to NeighborListe: " << nEntry->neighbor_addr.S_addr.getIPAddress().str() << "\n";
...
void PASER_Routing_Table::updateNeighborFromHELLO(address_list liste,
    ...
    netmask.S_addr = IPv4Address::ALLONES_ADDRESS.getInt();
    ev << "updating Kernel Routing Table from AddL, dest: "
            << liste.ipaddr.S_addr.getIPAddress().str() << ", metric: " << 1
            << "\n";
...
void PASER_Routing_Table::updateRouteFromHELLO(address_list liste, int ifIndex,
    ...
    netmask.S_addr = IPv4Address::ALLONES_ADDRESS.getInt();
    ev << "updating Kernel Routing Table from AddL, dest: "
            << liste.ipaddr.S_addr.getIPAddress().str() << ", metric: " << 2
            << "\n";
