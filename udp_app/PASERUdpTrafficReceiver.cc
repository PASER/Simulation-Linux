...
bool PASERUdpTrafficReceiver::isStatsInit = true;
int PASERUdpTrafficReceiver::receiverNumber = 0;
...
#ifndef No_init_manetStats
//	numberOfTotalSentPackets = inStats->getPacketSend();
	numberOfTotalSentPackets = inStats->getPacketSendToDest(NameDesKnotens);
#endif
...
#ifndef No_init_manetStats
	if(senderMap.size()!=0){
	    dataFile << pdr << "\t"                                            //"Packet Delivery Ratio"
	                << delay/senderMap.size() << "\t"                      //TotalDelay
	                << maxDelay/senderMap.size() << "\t"                   //MaxDelay
	                << minDelay/senderMap.size() << "\t"                   //MinDelay
	                << hopCount/senderMap.size() << "\t"                   //HopCount
	                << inStats->getPacketSend() << "\t"                    //numberOfTotalSentPackets
	                << inStats->getPacketBroadcastSend() << "\t"           //BroadcastMessages
	                << inStats->getPacketUnicastSend() << "\t"             //UnicastMessages
	                << inStats->getTotalSendProtokollBytes()               //ProtokollByteOverhead
	                << endl;
	}
	else{
        dataFile << pdr << "\t"                                             //"Packet Delivery Ratio"
...
#ifndef No_init_manetStats
    collDataFile << collStats::getInstance()->getUdpDataColl() << "\t"           //udpDataColl
                 << collStats::getInstance()->getProtocolColl() << "\t"          //protocolColl
                 << collStats::getInstance()->getMacColl() << "\t"               //macColl
                 << endl;
#endif
    collDataFile.close();
...
#ifndef No_init_manetStats
	numberOfTotalReceivedPackets = inStats->getPacketReceived();
	numberOfTotalSentPackets = inStats->getPacketSend();
#endif
	numberOfTotalReceivedPackets = numberOfOKReceivedPackets;
	double pdr = numberOfTotalReceivedPackets/numberOfTotalSentPackets;
